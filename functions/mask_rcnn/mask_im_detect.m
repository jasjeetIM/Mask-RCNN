function [pred_boxes, scores, masks] = mask_im_detect(conf, caffe_net, im, boxes, max_rois_num_in_gpu)
% Fast R-CNN
...
% --------------------------------------------------------
    ...
    total_masks = cell(ceil(total_rois / max_rois_num_in_gpu),1); 
    ...
    for i = 1:ceil(total_rois / max_rois_num_in_gpu)
        ...
        box_deltas = permute(box_deltas, [2,1]); 
 
        masks = caffe_net.blobs('binary_mask').get_data(); 
        
        total_masks{i} = masks; 
        total_scores{i} = scores;
        total_box_deltas{i} = box_deltas;
    end 
    masks = cell2mat(total_masks); 
