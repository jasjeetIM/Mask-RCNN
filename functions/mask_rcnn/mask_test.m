function mAP = mask_test(conf, imdb, roidb, varargin)
% --------------------------------------------------------
% Fast R-CNN
...
%%  set cache dir
    cache_dir = fullfile(pwd, 'output', 'mask_rcnn_cachedir', opts.cache_name, imdbs_name);
...
%%      testing 
        ...
        caffe_net = caffe.get_net(opts.net_def_file, opts.net_file, 'test')
        ...
        mask_results = cell(length(imdb.image_ids), 1); 
        count = 0;
        t_start = tic;
        for i = 1:num_images
            count = count + 1;
            ...
            [boxes, scores, masks] = mask_im_detect(conf, caffe_net, im, d.boxes, max_rois_num_in_gpu);
            mask_results{i} = masks; 
            for j = 1:num_classes
        ...
        save('mask_predictions.mat', 'mask_results');
        for j = 1:num_classes
            [aboxes{j}, box_inds{j}, thresh(j)] = ...
                keep_top_k(aboxes{j}, box_inds{j}, i, max_per_set, thresh(j));
        end
...
%%  try to determine the maximum number of rois
    ...
    fprintf('Starting GPU check for Testing Stage 1\n'); 
    max_rois_num = 0;
    rois_num = 500
        ...
        if gpuInfo.FreeMemory < 2 * 10^9  % 2GB for safety
          fprintf('Do not have 2GB extra GPU memory\n'); 
        end
end
