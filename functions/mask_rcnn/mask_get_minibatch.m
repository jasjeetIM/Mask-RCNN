function [im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob mask_targets_blob] = mask_get_minibatch(conf, image_roidb)
% --------------------------------------------------------
% Fast R-CNN
...
% --------------------------------------------------------
    ...
    mask_targets_blob = zeros(0, num_classes+1, 28,28, 'single'); 

    for i = 1:num_images
        [labels, ~, im_rois, bbox_targets, bbox_loss, mask_targets] = ...
        ...
        mask_targets_blob = [mask_targets_blob; mask_targets]; 
    end
    ...
    assert(~isempty(mask_targets_blob)); 
end
...
function [labels, overlaps, rois, bbox_targets, bbox_loss_weights, mask_targets] = ...
...
%     overlaps = image_roidb(1).max_overlaps;
    ...
    % Infer number of classes from the number of columns in gt_overlaps
    num_classes = size(image_roidb(1).overlap, 2);
    mask_targets = zeros(0, num_classes+1, 28, 28, 'single'); 

    for i=1:size(rois,1)
      mask_targets_roi = zeros(1,num_classes+1, 28, 28, 'single'); 
      mask_roi = image_roidb(1).mask_targets(i, :, :);
      objects = unique(mask_roi); 
      no_loss_mask = zeros(28,28); 
      no_loss_mask(:) = -1;  
      for j=1:num_classes
        % Prepare mask for loss. 
        % Count only if class exists in mask
        if any(objects == j)
          mask_targets_roi(1,j,:,:) = mask_roi == j; 
        else 
          mask_targets_roi(1,j,:,:) = no_loss_mask; 
        end
      end
      %Get mask for background class
      if any(objects == 0)
        mask_targets_roi(1,num_classes+1, :, : ) = mask_roi == 0; 
      else
        mask_targets_roi(1,num_classes+1, :, : ) = no_loss_mask; 
      end
      mask_targets = [mask_targets; mask_targets_roi]; 
    end
